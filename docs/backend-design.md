...
既存構成のLaravel 11、Inertia.js、Reactを前提とする。MVPでは独立したJSON APIを設けず、次の方式に統一する。
...
参加者本人のParticipationとPartEntryだけを操作できる。担当確定済みの応募取消はAssignmentを直接解除せず、参加キャンセルまたは運営者への連絡を案内する。
...
| `ReviewParticipation` | 承認または見送りと通知 |
| `SubmitPartEntry` | 承認状態、期限、応募上限、順位を検証 |
| `ReviewPartEntry` | 保留または見送りと通知 |
| `ConfirmAssignment` | 枠と担当上限をロック下で検証して確定 |
| `ReleaseAssignment` | 担当解除、再募集判定、通知 |
| `CancelParticipation` | 参加・応募・担当をまとめて取消 |
...
| イベント中止 | 承認済み参加者 | 必須 | 必須 |
| 日時・会場・参加費の変更 | 承認済み参加者 | 必須 | 必須 |
| お知らせ公開 | 承認済み参加者 | 必須 | 任意設定 |
| 再募集発生 | owner・staff | 必須 | 必須 |

メール送信はQueueへ投入し、業務トランザクションのコミット後に実行する。
...
- 未承認参加者はPartEntryを作成できない。
- 応募期限、応募数上限、担当曲数上限を超えられない。
- 同時に担当確定しても必要人数を超えない。
