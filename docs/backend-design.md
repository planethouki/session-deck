...
| PATCH | `/events/{event:slug}/entries/{entry}` | `events.entries.update` | 順位以外の応募内容変更 |
| DELETE | `/events/{event:slug}/entries/{entry}` | `events.entries.destroy` | 応募取消 |

参加者本人のParticipationとPartEntryだけを操作できる。個別PATCHで変更できるのは希望キー、コーラス・持ち替え可否、コメントに限定する。順位の変更・交換は一括更新ルートだけを使い、Participationをロックしたトランザクション内で行う。担当確定済みの応募取消はAssignmentを直接解除せず、参加キャンセルまたは運営者への連絡を案内する。

`entries/priorities`は`entries/{entry}`より前にルート定義し、`priorities`がモデルIDとして解決されないようにする。

## 通知ルート

`auth`と`verified`を必須とし、本人の通知だけを操作できる。

| Method | URL | Route name | 用途 |
| --- | --- | --- | --- |
| GET | `/notifications` | `notifications.index` | 通知一覧 |
| PATCH | `/notifications/{notification}/read` | `notifications.read` | 個別既読 |
| POST | `/notifications/read-all` | `notifications.read-all` | 全件既読 |

`notifications/read-all`は動的な`{notification}`ルートより前に定義する。
...
| GET | `/manage/events/{event:slug}/setlist` | `manage.events.setlist.edit` | セットリスト編集画面 |
...
- `Manage\SetlistController`：セットリスト表示・曲順更新
...
- `NotificationController`：本人の通知一覧・既読化
...
| `StorePartEntryRequest` | 枠、応募資格、期限、異なる応募曲数上限。順位は末尾へ自動設定 |
| `UpdatePartEntryRequest` | 本人所有、未確定、希望キー、対応可否、コメント。順位は受け付けない |
...
| `StoreSongResourceRequest` | URL、資料種別、公開範囲、タイトル |
| `StoreAnnouncementRequest` | タイトル、本文 |
| `PublishAnnouncementRequest` | メール送信有無、未公開であること |
...
| `UpdateEventSong` | 曲情報を更新し、公開済みの重要項目変更を担当者へコミット後通知 |
| `SubmitParticipation` | 重複・期限・定員を検証して申込 |
| `ReviewParticipation` | Eventをロックし、承認済み参加者数と定員を再確認して承認または見送り |
| `SubmitPartEntry` | Participationをロックし、応募資格と異なる応募曲数上限を再検証して末尾順位で作成 |
| `UpdatePartEntryPriorities` | Participationをロックし、希望順位を二段階で一括更新 |
| `CancelPartEntry` | Participationをロックして取消し、残る有効応募の順位を圧縮 |
| `ReviewPartEntry` | Participationをロックし、保留または見送り。見送り時は順位を圧縮 |
| `ConfirmAssignment` | Participation、PartSlotの順にロックし、異なる担当曲数上限と枠を再検証してAssignment履歴を作成 |
...
| `PublishAnnouncement` | メール送信設定と公開日時を確定し、対象者への通知をコミット後に投入 |

Actionの公開メソッドは原則`handle(...)`に統一し、Eloquent Modelまたは結果オブジェクトを返す。

### Action共通のロック順序

複数行をロックするActionは、デッドロックを避けるため`Event → Participation → PartSlot → PartEntry / Assignment`の順に統一する。同種の行を複数ロックする場合はID昇順とする。

- `SubmitPartEntry`はParticipationをロック後、異なる応募曲数を再集計する。
- `ConfirmAssignment`はParticipation、PartSlotの順にロック後、異なる担当曲数と枠の空きを再集計する。
- `CancelPartEntry`と応募見送りはParticipationをロックし、状態変更と順位圧縮を同一トランザクションで処理する。
...
- `ExperienceLevel`
- `SongResourceType`
- `SongResourceVisibility`

Enumには表示名を返す`label(): string`を持たせ、画面側で英語の保存値を直接表示しない。

`SongResourceVisibility`は`Public = 'public'`と`Participants = 'participants'`を持つ。`StoreSongResourceRequest`と更新用Requestは`Rule::enum(SongResourceVisibility::class)`を使う。SongResource Modelは`type`を`SongResourceType`、`visibility`を`SongResourceVisibility`へcastする。
...
EventSong、SongPartSlot、Announcementの更新は、親Eventの運営権限を基準に認可する。

### SongResourcePolicy

まずSongResource、EventSong、Eventの親関係が整合していることを確認する。不整合、親の欠落、未知の公開範囲、判定不能な状態では拒否する。

- `view`（一般閲覧）：`visibility = Public`、親Eventが公開対象、親EventSongが`is_published = true`のすべてを満たす場合だけ許可する。
- `view`（参加者）：親Eventが参加者向け閲覧対象、親EventSongが`is_published = true`で、対象イベントに現在`approved`のParticipationを持つ本人だけに許可する。
- `view`（運営者）：親Eventのownerまたはstaffには、下書きイベント・非公開曲を含めて許可する。
- `create`：対象EventSongの親Eventのownerまたはstaffだけに許可する。
- `update` / `delete`：既存SongResourceの親Eventのownerまたはstaffだけに許可する。

親Eventの一般公開対象は、`published_at`がNULLではなく、状態が`published`、`closed`、`completed`のいずれかであることとする。参加者向け閲覧対象も同じ状態とし、`draft`と`cancelled`では許可しない。一般公開判定とイベント公開クエリは共通のModel scopeまたはDomain Policyを使い、条件を重複実装しない。
...
イベント詳細では、公開情報、曲、パート枠、集計済み状態、`visibility = public`の演奏資料だけを返し、参加者限定資料や応募者の個人情報は返さない。承認済み参加者ページでは閲覧可能な全資料を返す。主催者の編成表では応募者プロフィールを返せるが、メールアドレスなど認証情報は除外する。
...
| 演奏キー・曲構成・演奏メモの変更 | その曲に有効なAssignmentを持つ参加者 | 必須 | 必須 |
...
公開時のメール送信設定はAnnouncementへ保存する。公開済みのお知らせは更新・削除できず、訂正は新しいお知らせとして公開する。

`UpdateEventSong`は`performance_key`、`structure_note`、`performance_note`の変更を検出する。イベント状態が`published`または`closed`で変更前後に差があり、その曲に有効なAssignmentがある場合、担当者をユーザー単位で重複排除して通知する。通知Jobは更新トランザクションのコミット後に投入する。曲名、アーティスト名など上記以外の変更ではこの通知を送らない。
...
- 取消・見送り後は、残る有効応募の順位が1から始まる連番へ自動圧縮される。
- 同じ参加者が異なる曲へ同時応募しても、異なる応募曲数上限を超えない。
- 同じ参加者を異なる枠へ同時確定しても、異なる担当曲数上限を超えない。
- 同時に担当確定しても必要人数を超えない。
- 担当解除後に再確定すると新しいAssignment履歴が作られ、過去履歴も残る。
- キャンセル後に有効な応募・担当が残らない。
- 公開イベントの応募者情報が一般閲覧者へ漏れない。
- 重要項目変更と状態変更の通知がコミット後に送信される。
- `published`または`closed`イベントの演奏キー・曲構成・演奏メモ変更が、その曲の有効な担当者だけへ重複なく通知される。
- 一般公開ページには`public`資料だけが含まれ、参加者限定資料はapproved参加者、owner、staffだけが閲覧できる。
- 資料公開範囲は`SongResourceVisibility`で検証・castされ、未知の値では保存と閲覧の両方が拒否される。
- 下書き・開催中止イベントまたは非公開曲の`public`資料は、一般ユーザーと参加者が個別URLからも取得できない。
- SongResourceの作成・更新・削除は親イベントのownerまたはstaffだけが実行できる。
- セットリスト編集GETルートが正しいInertiaページとPropsを返す。
