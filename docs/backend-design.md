...
EventSong、SongPartSlot、Announcementの更新は、親Eventの運営権限を基準に認可する。SongResourcePolicyは`SongResourceVisibility`で分岐し、`Public`なら全員、`Participants`なら対象イベントのapproved参加者、owner、staffだけに閲覧を許可する。未知の値や判定不能な状態の既定動作は拒否とし、権限を付与しない。
