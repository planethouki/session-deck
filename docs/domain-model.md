...
応募から確定した実際の担当。PartEntryの状態だけで確定を表現せず、Assignmentを独立させて編成と確定履歴を明確にする。
...
担当確定時にAssignmentを作成する。確定解除時はAssignmentに解除日時を記録し、応募を必要に応じて`applied`または`cancelled`へ戻す。
...
- 応募曲数と担当曲数の上限はイベントごとに設定でき、未設定時は無制限とする。
